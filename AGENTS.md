...
`anti-locker` — a macOS-only Go CLI that keeps the Mac awake via `caffeinate -i` only when connected to a trusted Wi-Fi network (configured by SSID). See [README.md](README.md) for usage and [docs/superpowers/specs/2026-08-01-anti-locker-design.md](docs/superpowers/specs/2026-08-01-anti-locker-design.md) for the design spec.
...
- Never commit secrets or the generated `anti-locker` binary (see `.gitignore`).
